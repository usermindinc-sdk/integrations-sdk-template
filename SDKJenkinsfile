    ...
    try {
        ...
        stage('Configure environment') {
            ...
            if(util.isPullRequest() || env.BRANCH_NAME == 'develop' || env.BRANCH_NAME == 'master') {
                ...
                slackTeamFailureMessageDestination = "#integration-build-failures"
            }
            gitCommit = util.commitSha()
        }

        def endAfterReleaseStage = false
        stage('Continuation and gitflow checks') {
             if (env.BRANCH_NAME != 'master' && currentBuild.getBuildCauses().toString().contains('hudson.model.Cause\$UpstreamCause')) {
                //If this is triggered by an upstream event, then only build master. No one else should do anything at all.
                 //But note that building master will trigger jgitflow, which will rebuild develop
                 echo "This was triggered by an upstream event, only going to build master"
                endAfterReleaseStage = true
            } else if (env.BRANCH_NAME == 'master' && !util.changeLogs().contains("with non-snapshot versions*  _by jenkins_"))  {
                echo "Running jgitflow"
                //withCredentials([usernamePassword(credentialsId: 'Github-Tony', passwordVariable: 'gpass', usernameVariable: 'guser')]) {
                withCredentials([usernamePassword(credentialsId: 'github-userminddeployer', passwordVariable: 'githubPassword', usernameVariable: 'githubUsername')]) {
                    sh 'mvn --batch-mode jgitflow:release-start jgitflow:release-finish -DpullDevelop=true -DnoDeploy=true -DpushReleases=true -DnoReleaseBuild=true'
                }
                endAfterReleaseStage = true
            }
        }

        if( endAfterReleaseStage == true ) {
        ...
        try {
            ...
            util.sendSlackMessage(slackTeamFailureMessageDestination, ":jenkins_general_rage: The sonar build failed!", "danger")
        }
